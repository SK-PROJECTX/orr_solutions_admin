    ...
    <div className="min-h-screen text-white relative overflow-hidden star">
      ...
      <div className="relative z-10 p-4 md:p-8">
        <div className="bg-card backdrop-blur-sm rounded-2xl p-4 md:p-8 flex flex-col gap-6 md:gap-8 border border-white/10 shadow-2xl">
          <div>
            <h1 className="text-2xl md:text-4xl font-bold text-white">{t('content_management.living_systems_title')}</h1>
            <p className="text-gray-400 text-xs md:text-sm mt-2">{t('content_management.living_systems_subtitle')}</p>
          </div>
          ...
          <div className={sectionClass}>
            ...
            <form onSubmit={(e) => { 
              ...
              />
              <RichTextEditor
                label={t('content_management.process_step', { num: 1 })}
                value={content.process_step_1 || ''}
                onChange={(value) => handleRichTextChange('process_step_1', value)}
                placeholder={t('content_management.process_step', { num: 1 })}
                rows={3}
              />
              <RichTextEditor
                label={t('content_management.process_step', { num: 2 })}
                value={content.process_step_2 || ''}
                onChange={(value) => handleRichTextChange('process_step_2', value)}
                placeholder={t('content_management.process_step', { num: 2 })}
                rows={3}
              />
              <RichTextEditor
                label={t('content_management.process_step', { num: 3 })}
                value={content.process_step_3 || ''}
                onChange={(value) => handleRichTextChange('process_step_3', value)}
                placeholder={t('content_management.process_step', { num: 3 })}
                rows={3}
              />
              <div className="flex gap-3 pt-4">
