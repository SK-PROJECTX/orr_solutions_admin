    ...
    <div className="min-h-screen text-white relative overflow-hidden star">
      ...
      <div className="relative z-10 p-4 md:p-8">
        <div className="bg-card backdrop-blur-sm rounded-2xl p-4 md:p-8 border border-white/10">
          <div className="mb-8">
            <h1 className="text-2xl md:text-4xl font-bold text-white mb-2">{t('meetings.upcoming_title')}</h1>
            <p className="text-gray-400">{t('meetings.upcoming_subtitle')}</p>
          </div>
          ...
          {loading ? (
            ...
            return allMeetings.length === 0 ? (
              <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">{t('meetings.no_upcoming')}</h3>
                <p className="text-gray-400">{t('meetings.no_upcoming_desc')}</p>
                {error && (
                  ...
                  {paginatedMeetings.map((meeting) => (
                    <div key={meeting.id} className="bg-gradient-to-r from-white/10 to-white/5 rounded-xl border border-white/10 p-6">
                      ...
                      <div className="grid md:grid-cols-2 gap-4 mb-4">
                        <div>
                          <label className="text-xs text-gray-400 mb-1 block">{t('meetings.scheduled_time')}</label>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-white/10">
                        <div className="text-xs text-black">
                          {t('dashboard.table_status')}: <span className="text-green-400">{t('meetings.confirmed')}</span>
                        </div>
