...
import { Calendar, FileText, User, Loader, CheckCircle, Clock, MapPin, ExternalLink } from "lucide-react";
  ...
  useEffect(() => {
    const fetchPastMeetings = async () => {
      try {
        ...
        setError(t('consultations.error_loading_past'));
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen text-white relative overflow-hidden star">
      ...
      <div className="relative z-10 p-4 md:p-8">
        <div className="bg-card backdrop-blur-sm rounded-2xl p-4 md:p-8 border border-white/10">
          <div className="mb-8">
            <h1 className="text-2xl md:text-4xl font-bold text-white mb-2">{t('consultations.past_page_title')}</h1>
            <p className="text-gray-400">{t('consultations.past_page_subtitle')}</p>
          </div>
          ...
          {loading ? (
            ...
            return allMeetings.length === 0 ? (
              <div className="bg-white/5 border border-white/10 rounded-xl p-6 text-center">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">{t('consultations.no_past')}</h3>
                <p className="text-gray-400">{t('consultations.no_past_desc')}</p>
              </div>
            ) : (
              <>
                <div className="mb-6">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          {meeting.meeting_link && (
                            <div className="bg-blue-500/10 rounded-lg p-3 mb-3 border border-blue-500/20 w-fit">
                          ...
                          {meeting.meeting_notes && (
                            <div className="bg-white/5 rounded-lg p-3">
                              <p className="text-sm text-gray-300">
                                <strong>{t('consultations.meeting_notes')}:</strong> {meeting.meeting_notes}
                              </p>
                            </div>
                          )}
                        </div>
